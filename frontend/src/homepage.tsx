...
function Nav({ onLogin, onDashboard, username }: { onLogin: () => void; onDashboard: () => void; username?: string }) {
  return (
    <nav style={{
      ...
      padding: '20px 48px', position: 'relative', zIndex: 20,
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{
          ...
          <div style={{ width: 16, height: 16, borderRadius: '50%', background: '#FFE8D9' }} />
        </div>
        <span style={{ fontWeight: 900, fontSize: 20, letterSpacing: -0.4 }}>Bloom</span>
...
function Hero({ onLogin, onAnalytics }: { onLogin: () => void; onAnalytics: () => void }) {
  ...
  useEffect(() => {
    let i = 0;
    const id = setInterval(() => { i = (i + 1) % msgs.length; setBlobMsg(msgs[i]); }, 5000);
  ...
  return (
    <section style={{
      ...
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0 }}>
        <div style={{ position: 'absolute', top: '-10%', right: '-5%', width: 600, height: 600, borderRadius: '50%', background: 'radial-gradient(circle,rgba(255,232,217,0.7) 0%,transparent 70%)' }} />
        <div style={{ position: 'absolute', bottom: '-5%', left: '-8%', width: 500, height: 500, borderRadius: '50%', background: 'radial-gradient(circle,rgba(255,234,184,0.55) 0%,transparent 70%)' }} />
      </div>

      <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', maxWidth: 720 }}>
        {/* Badge */}
        <div className="fade-up fade-up-1" style={{ marginBottom: 24, display: 'flex', justifyContent: 'center' }}>
          <div className="landing-pill" style={{ background: C.accentSoft, color: C.accent }}>
            ✦ Flicker to Flow
          </div>
        </div>
        ...
        <p className="fade-up fade-up-3" style={{
          ...
          color: C.soft, maxWidth: 540, margin: '0 auto 36px',
        }}>
          Bloom watches your eye signals and computer behaviour to catch micro-distractions before they steal your flow — and celebrates every win with you.
        </p>
        ...
        {/* Mascot */}
        <div className="fade-up fade-up-4" style={{ display: 'flex', justifyContent: 'center', position: 'relative' }}>
          <div className="hero-blob" style={{ position: 'relative', display: 'inline-block' }}>
            <Blob state="encouraging" palette="cream" shape="wide" size={180} showGround />
            <div style={{
              position: 'absolute', left: 'calc(100% + 8px)', top: 10,
              background: C.card, border: `2px solid ${C.border}`,
              borderRadius: 18, padding: '10px 14px',
              fontSize: 13, fontWeight: 700, color: '#5a3e2a',
              ...
              {blobMsg}
              <svg width="14" height="14" viewBox="0 0 14 14" style={{ position: 'absolute', bottom: 8, left: -10 }}>
                <path d="M14 0 L 14 14 L 0 7 Z" fill={C.card} />
                <path d="M14 0 L 0 7 L 14 14" fill="none" stroke={C.border} strokeWidth="2" strokeLinejoin="round" />
              </svg>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 48, flexWrap: 'wrap' }}>
          {['Eye Signal tracking', 'Computer behaviour', 'Coin rewards', 'Deep analytics'].map(label => (
            <div key={label} style={{
              padding: '10px 20px',
              border: `1.5px solid ${C.border}`,
              borderRadius: 12,
              background: C.card,
              fontSize: 13, fontWeight: 800, color: C.soft,
              letterSpacing: -0.2,
...
function Features() {
  const features = [
    ...
    },
    {
      ...
      items: ['Custom time windows', 'App & tab presets', 'Coin rewards', 'Session-end insights'],
    },
  ];
  ...
  return (
    <section style={{ padding: '80px 48px', maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: 52 }}>
        <div className="landing-pill" style={{ background: C.accentSoft, color: C.accent, marginBottom: 16 }}>How it works</div>
        <h2 style={{ fontSize: 40, fontWeight: 900, letterSpacing: -1.2, marginBottom: 12, color: C.ink }}>Three layers of intelligence</h2>
        <p style={{ color: C.soft, fontWeight: 500, fontSize: 16, maxWidth: 480, margin: '0 auto' }}>
          Bloom fuses biosignals and computer behaviour into a single focus score.
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
        {features.map(f => (
          <div key={f.title} className="landing-feature-card">
            ...
            <h3 style={{ fontSize: 20, fontWeight: 900, letterSpacing: -0.4, marginBottom: 8, color: C.ink }}>{f.title}</h3>
            <p style={{ fontSize: 14, fontWeight: 500, color: C.soft, lineHeight: 1.6, marginBottom: 18 }}>{f.desc}</p>
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 7 }}>
              {f.items.map(it => (
                <li key={it} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 700, color: C.ink }}>
  ...
  return (
    <section style={{
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
        <div>
          <div className="landing-pill" style={{ background: C.accentSoft, color: C.accent, marginBottom: 20 }}>End-session insights</div>
          <h2 style={{ fontSize: 38, fontWeight: 900, letterSpacing: -1.2, lineHeight: 1.1, marginBottom: 18, color: C.ink }}>
            Know exactly<br />where you drifted.
          </h2>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
            <div>
              <div style={{ fontSize: 12, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1.4, color: C.soft, marginBottom: 4 }}>Focus score</div>
              <div style={{ fontSize: 52, fontWeight: 900, letterSpacing: -2, lineHeight: 1, color: C.ink }}>84</div>
          ...
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 12, fontWeight: 800, color: C.soft, textTransform: 'uppercase', letterSpacing: 1.2, marginBottom: 8 }}>Top break patterns</div>
            {breaks.map(b => (
              <div key={b.rank} style={{ marginBottom: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, fontWeight: 700, marginBottom: 3, color: C.ink }}>
                  <span>{b.rank}. {b.label}</span>
                  <span style={{ color: C.soft }}>{b.pct}%</span>
                </div>
                <div style={{ height: 6, borderRadius: 999, background: C.accentSoft }}>
                  <div style={{ height: '100%', borderRadius: 999, background: b.color, width: `${b.pct}%` }} />
                </div>
              </div>
            ))}
          </div>

          <div style={{ background: C.accentSoft, borderRadius: 12, padding: '10px 14px', fontSize: 12, fontWeight: 700, color: C.ink }}>
...
function FooterCTA({ onLogin }: { onLogin: () => void }) {
  return (
    <section style={{ padding: '100px 48px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 60%,rgba(240,143,96,0.12) 0%,transparent 65%)', pointerEvents: 'none' }} />
      <div style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 28 }}>
          <Blob state="cheering" palette="cream" shape="wide" size={120} showGround />
        </div>
        <h2 style={{ fontSize: 44, fontWeight: 900, letterSpacing: -1.5, marginBottom: 14, color: C.ink }}>
          Ready to find your flow?
        </h2>
        <p style={{ fontSize: 16, fontWeight: 500, color: C.soft, marginBottom: 36 }}>
          Join Bloom and turn every session into a win.
        </p>
        <button className="landing-cta-btn" style={{ fontSize: 18, padding: '18px 44px' }} onClick={onLogin}>
          Start for free →
        </button>
      </div>
    </section>
