...
function RegisterPage() {
  ...
  const handleGoogle = async () => {
    ...
    catch (err: unknown) { setError((err as { message?: string })?.message ?? "Google sign-in failed."); }
  };

  const handleGitHub = async () => {
    setError("");
    try { await loginWithGitHub(); }
    catch (err: unknown) { setError((err as { message?: string })?.message ?? "GitHub sign-in failed."); }
  };
      ...
      >
        ...
        <div className="relative z-10 flex flex-1 items-end justify-center pb-10">
          <div className="flex items-end gap-10">
            <div ref={peachieRef}>
              <Blob state={getCompanionState(0)} palette="peach" shape="classic" size={135} eyeTarget={eyeTarget} lean={computeLean(peachieRef)} showGround />
            </div>
            <div ref={pudgeRef}>
              <Blob state={getPudgeState()} palette="cream" shape="wide" size={215} eyeTarget={eyeTarget} lean={computeLean(pudgeRef)} showGround />
            </div>
            <div ref={bubsRef}>
              <Blob state={getCompanionState(1)} palette="honey" shape="baby" size={115} eyeTarget={eyeTarget} lean={computeLean(bubsRef)} showGround />
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center justify-center p-8 bg-background">
        <div className="w-full max-w-sm">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground text-sm font-semibold">Start your focus journey today</p>
          </div>
          ...
          <p className="text-center text-sm text-muted-foreground mt-6 font-semibold">
            ...
            <button onClick={() => navigate("/login")} className="font-black hover:underline cursor-pointer" style={{ color: "#3D2A1B" }}>
              Sign in
            </button>
          </p>
        </div>
