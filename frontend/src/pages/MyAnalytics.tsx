...
function HourlyChart({ hourBuckets, peakHour }: {
  hourBuckets: Record<number, { avg: number; count: number }>;
  peakHour: number;
}) {
  const W = 480, H = 80;
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display: "block" }}>
      {Array.from({ length: 24 }, (_, h) => {
        ...
        const fill = isPeak ? C.accent : b ? scoreColor(avg) : C.border;
        const opacity = isPeak ? 1 : b ? 0.75 : 0.35;
        return (
          <rect key={h} x={x + 1} y={y} width={barW - 2} height={barH}
      ...
      {labelHours.map(h => (
        <text key={h}
          ...
          y={H - 2}
          textAnchor="middle" fontSize={8} fill={C.soft}
        >
          {hourLabel(h)}
        </text>
      ))}
...
function OverviewTab({
  ...
  const { sessions, totalSessions, avgScore, totalCoins, peakHours, streak, hourBuckets } = data;
    ...
    >
      ...
      {peakHours.length > 0 && (
        <div className="ma-card" style={{ padding: "20px 22px" }}>
          <div style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: 1.2, color: C.soft, marginBottom: 12 }}>
          ...
          {/* 24-hour chart */}
          <HourlyChart hourBuckets={hourBuckets} peakHour={peakHours[0].hour} />
        </div>
      )}
      ...
      <div>
        ...
        {sessions.length > 6 && (
          ...
          >
            {sessions.length - 6} more session
            {sessions.length - 6 !== 1 ? "s" : ""} not shown
          </div>
        )}
        {sessions.length === 0 && (
          <button
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Each distraction type has a{" "}
          <strong style={{ color: C.ink }}>weight</strong> based on how much it
          disrupts deep focus. Impact = count × weight. Microsleeps score
          highest at 10×, small distractions like head tilts score 1×.
        </p>
      </div>
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div
                        style={{ fontSize: 11, fontWeight: 700, color: C.soft }}
                      >
                        {d.count} event{d.count !== 1 ? "s" : ""} · weight{" "}
                        {WEIGHTS[d.type] ?? 1}×
                      </div>
                    </div>
                  </div>
...
function TipsTab({ data }: { data: TabData }) {
  ...
  const personalTips = topDistractions.map((d) => TIPS[d.type]).filter(Boolean);
  ...
  let profileSummary =
    "You're making good progress. Here's a personalised plan to take your focus further:";
  if (top1) {
...
export default function MyAnalytics() {
  ...
  return (
    ...
    >
        ...
        >
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ fontSize: 12, fontWeight: 800, color: C.soft }}>
              <span style={{ color: C.ink, fontSize: 20, fontWeight: 900 }}>
                {totalSessions}
              </span>{" "}
            ...
            >
              <Coins size={14} />
              <span style={{ color: C.ink, fontSize: 18, fontWeight: 900 }}>
                {totalCoins}
              </span>{" "}
              coins
            </div>
      ...
      >
        {activeTab === "overview" && (
          <OverviewTab data={tabData} navigate={navigate} />
        )}
