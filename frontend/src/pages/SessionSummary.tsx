...
const TIPS: Record<string, string> = {
  microsleep:      "Try a 5-min break and some cold water.",
  yawn:            "Fatigue detected — consider a short nap or stretching.",
  phone_check:     "Keep your phone face-down or in another room.",
  head_tilt:       "Check your monitor height — it should be at eye level.",
  eyes_off_screen: "Stay in frame or adjust your camera position.",
  tab_switch:      "Use a site blocker to reduce tab temptation.",
  disallowed_tab:  "Remove distracting bookmarks from your toolbar.",
  rewind:          "Try active note-taking to stay engaged.",
};
...
export default function SessionSummary() {
  ...
  const grade =
    summary.focus_score >= 90 ? { label: "Excellent!",  sub: "Pudge is so proud of you.",      color: "text-emerald-400", blob: "cheering"    as const } :
    summary.focus_score >= 70 ? { label: "Good Job!",   sub: "A solid session — keep it up.",   color: "text-amber-400",   blob: "encouraging" as const } :
                                 { label: "Keep At It", sub: "Every session makes you stronger.", color: "text-red-400",     blob: "sad"         as const };

  const blobBase = entered ? grade.blob : "cheering";
  const { blobState, onPet } = usePettable(blobBase);

  const topDistractors = summary.top_distractors.length > 0
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-2xl mx-auto px-8 py-10">
        ...
        {topDistractors.length > 0 && (
          <div className="mb-8">
            <h2 className="text-base font-bold mb-3 text-muted-foreground uppercase tracking-wider text-sm">
              What tripped you up
            </h2>
                ...
                return (
                  <div key={d.type} className="rounded-xl border border-border/40 bg-card p-4">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-semibold text-sm">{DISTRACTOR_LABELS[d.type] ?? d.type}</span>
                      <span className="text-xs text-muted-foreground">
                        ×{d.count}{d.impact > 0 ? ` · -${d.impact}pts` : ""}
                      </span>
                    </div>
        ...
        {topDistractors.length === 0 && (
          <div className="mb-8 rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-5 text-center">
            <p className="text-emerald-400 font-semibold">Zero distractions detected!</p>
            <p className="text-muted-foreground text-sm mt-1">Pudge is very impressed.</p>
          </div>
        )}

        {/* Actions */}
        <div className="flex gap-3">
          <Button onClick={() => navigate("/start")} className="flex-1 h-12 font-semibold gap-2">
            <RotateCcw className="size-4" /> New Session
          </Button>
          <Button onClick={() => navigate("/")} variant="outline" className="flex-1 h-12 gap-2">
